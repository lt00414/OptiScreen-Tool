...
ninps = len(label_states + low_states + high_states) + 5
print("ninps = ", ninps)
...
layout = html.Div(
    [
        html.Table([inp_code_hitwell, inp_nvars, inp_nsamples]),
        controls_html,
        btn_compute,
        #graph, hover_info,
    ],
...
@app.callback(
    Output('output_code_hitwell', 'children'),
    [Input('submit-button', 'n_clicks')],
    [State('inp_code', 'value'), 
     State('inp_hitwell', 'value')])
def update_output_code_hitwell(n_clicks, input_code, input_hitwell):
    return (input_code, input_hitwell)
